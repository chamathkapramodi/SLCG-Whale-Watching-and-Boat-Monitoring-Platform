...
export function AdminRecordsProvider({ children }: { children: ReactNode }) {
 ...
 useEffect(()=>{
  ...
  let active=true;
  void Promise.all([operationsApi.boats(session.accessToken),operationsApi.trips(session.accessToken),operationsApi.directory(session.accessToken)]).then(([apiBoats,apiTrips,directory])=>{
   if(!active)return;
