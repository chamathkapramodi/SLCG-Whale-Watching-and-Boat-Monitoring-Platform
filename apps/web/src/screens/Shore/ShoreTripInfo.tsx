...
function DataCard({ title, columns, rows, emptyText }: { title: string; columns: string[]; rows: string[][]; emptyText:string }) { const [search,setSearch]=useState(""); const visible=useMemo(()=>{const query=search.trim().toLowerCase();return query?rows.filter(row=>row.some(value=>value.toLowerCase().includes(query))):rows;},[rows,search]); return <section className="overflow-hidden rounded-xl bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-100 px-6 py-5"><h2 className="text-lg font-semibold text-[#14223d]">{title}</h2><input aria-label={`Search ${title}`} value={search} onChange={event=>setSearch(event.target.value)} placeholder="Search" className="w-28 rounded-md border border-slate-200 px-3 py-2 text-xs outline-none focus:border-indigo-400 sm:w-40" /></div><div className="overflow-x-auto"><table className="w-full min-w-[560px] text-left text-xs"><thead className="bg-slate-50 text-slate-500"><tr>{columns.map((column) => <th key={column} className="px-6 py-3 font-medium">{column}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{visible.map((row, index) => <tr key={index}>{row.map((value, cell) => <td key={cell} className="px-6 py-4 text-slate-600">{value}</td>)}</tr>)}</tbody></table>{visible.length===0&&<p className="px-6 py-8 text-center text-xs text-slate-400">{search?"No matching records.":emptyText}</p>}</div></section>; }
