...
async function request<T>(path:string, token:string, init?:RequestInit):Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(response.status === 403 ? 'You are not permitted to perform this action.' : validation ?? problem?.detail ?? problem?.title ?? `Request failed (${response.status}).`);
  }
...
export const operationsApi = {
  ...
  createTrip:(token:string,boatId:string,scheduledDepartureUtc:string,crewUserIds:string[])=>request<{id:string}>('/api/operations/trips',token,{method:'POST',body:JSON.stringify({boatId,scheduledDepartureUtc,route:'To be confirmed',passengerCount:0,crewUserIds})}),
